...
import { initPostboxDownloader } from './features/postboxDownloader/PostboxDownloader';
import { processTimestamps, parseTime, updatePerformanceIndicator, removeAllIndicators } from './features/latencyMonitor/LatencyMonitor';
import { injectConfirmPagePerformanceInfo, ensureConfirmPageWrapper } from './features/positionPerformance/PositionPerformance';
...
// Helper functions moved to features/latencyMonitor
...
export const injectLimitAdjuster = () => {
...
export const injectLimitButtons = () => {
  injectStyles();
...
export const injectConfirmPageButtons = () => {
...
// injectConfirmPagePerformanceInfo moved to features/positionPerformance

// Position Performance moved to features/positionPerformance
...
// Latency Monitor moved to features/latencyMonitor

// Run every second
setInterval(() => processTimestamps(settings), 1000);

// Run immediately on load
processTimestamps(settings);
// Postbox Downloader moved to features/postboxDownloader

// Initialize at end of script
setTimeout(() => {
  deactivateFixedPriceMode(); // Always start with Fix mode off
  initPostboxDownloader(settings.postboxDownloaderEnabled, settings.postboxFilenameMode);
}, 1000);
